<script lang="ts">
	...
	import { BackgroundTaskStatus, type BackgroundTask } from '../BackgroundTaskList.svelte';
	...
	<div class="background-task">
		<div class="name">
			...
			{:else if $backgroundTask.status == BackgroundTaskStatus.Ready}
				<button on:click={() => $backgroundTask.run()} title="Run"><PlayIcon size="16px" /></button>
				<button on:click={() => $backgroundTask.dismiss()} title="Dismiss">
					<XIcon size="16px" />
				</button>
			{/if}
