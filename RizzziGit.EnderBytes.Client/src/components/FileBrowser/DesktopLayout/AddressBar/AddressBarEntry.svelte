<script lang="ts">
	...
	import { FolderIcon, ChevronRightIcon, UsersIcon } from 'svelte-feather-icons';
	import Awaiter from '../../../Bindings/Awaiter.svelte';
	import Overlay, { OverlayPositionType } from '../../../Widgets/Overlay.svelte';
...
<div class="path-entry">
	...
	{:else}
		<button class={index == 0 ? 'share-button' : ''} on:click={() => goto(`/app/files/${file.id}`)}>
			{#if index == 0}
				<UsersIcon size="18em" />
			{/if}
			<p>{file.name}</p>
...
{#if menuShown}
	...
	>
		<div class="menu">
			<Awaiter
				callback={() => apiFetch({ path: `/file/${file != null ? `:${file.id}` : '!root'}/files` })}
			>
