...
public sealed partial class FileManager
{
    public sealed class InvalidNameException(
        Resource folder,
        string name,
        FileNameVaildationFlag flag
    ) : Exception($"Invalid name: '{name}'. Flag {flag}")
    {
    ...
    public async Task<FileNameVaildationFlag> ValidateName(
            ...
            await Count(
                ...
                new WhereClause.Nested(
                    ...
                    new WhereClause.CompareColumn(
                        COLUMN_DOMAIN_USER_ID,
                        "=",
                        parentFolder.AuthorUserId
                    ),
    ...
    public async Task<string> ThrowIfInvalidName(
        ResourceService.Transaction transaction,
        Resource parentFolder,
        string name
    )
