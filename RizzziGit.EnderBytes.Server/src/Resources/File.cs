...
public sealed partial class FileManager : ResourceManager
{
	...
	private const string DELETE_CONTEXT_USER_AUTHENTICATION_TOKEN = $"{NAME}_Delete_UserAuthenticationToken";
	...
	public sealed class InvalidRootDeleteException(Resource file) : Exception($"#{file.Id} cannot be deleted because it is a root folder.");
	...
	public FileManager(ResourceService service) : base(service, NAME, VERSION)
	{
		...
		RegisterDeleteHandler(async (transaction, resource) => await Delete(transaction, new WhereClause.CompareColumn(COLUMN_PARENT_ID, "=", resource.Id)));
	}
	...
	public IAsyncEnumerable<Resource> PathChain(ResourceService.Transaction transaction, Resource file)
	{
	...
	public async Task<bool> Move(ResourceService.Transaction transaction, Resource file, Resource newParent, string? newName, UserAuthenticationToken userAuthenticationToken)
	{
		...
		Resource[] pathChain = await PathChain(transaction, newParent).ToArrayAsync();
	...
	// public async Task<bool> ListTrashed(ResourceService.Transaction transaction, UserManager.Resource user, Resource? parentFolder = null, LimitClause? limitClause = null, OrderByClause[]? orderByClause = null)
	// {
	...
	//	 parentFolder == null ? null : new WhereClause.CompareColumn(COLUMN_PARENT_ID, "=", parentFolder.Id)
	...
	public async IAsyncEnumerable<Resource> ScanFolder(ResourceService.Transaction transaction, Resource folder, UserAuthenticationToken userAuthenticationToken, LimitClause? limitClause = null, OrderByClause[]? orderByClause = null, bool? isFolder = null)
	{
		if (!folder.IsFolder)
		{
			throw new NotAFolderException(folder);
	...
	public async Task<bool> Delete(ResourceService.Transaction transaction, Resource resource, UserAuthenticationToken userAuthenticationToken)
	{
		...
		KeyService.AesPair oldParentKey = await GetKeyRequired(transaction, oldParent, FileAccessExtent.ReadWrite, userAuthenticationToken);
		if (resource.TrashTime != null)
		{
			Resource root = await GetRootFromUser(transaction, userAuthenticationToken);
			KeyService.AesPair rootKey = await GetKeyRequired(transaction, root, FileAccessExtent.ReadWrite, userAuthenticationToken);

			await Update(transaction, resource, new(
