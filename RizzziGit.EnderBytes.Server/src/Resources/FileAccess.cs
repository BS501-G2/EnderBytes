...
public sealed class FileAccessManager : ResourceManager
{
	...
	public async Task<Resource> GrantUser(ResourceService.Transaction transaction, FileManager.Resource file, UserManager.Resource targetUser, KeyService.AesPair fileKey, UserManager.Resource authorUser, FileAccessExtent extent = FileAccessExtent.ReadOnly)
	{
	...
	public IAsyncEnumerable<Resource> List(ResourceService.Transaction transaction, FileManager.Resource? file = null, UserManager.Resource? targetUser = null, FileAccessExtent? extent = null, UserManager.Resource? authorUser = null, LimitClause? limit = null, OrderByClause[]? orderBy = null)
	{
		return Select(transaction, new WhereClause.Nested("and",
			...
			authorUser != null ? new WhereClause.CompareColumn(COLUMN_AUTHOR_USER_ID, "=", authorUser.Id) : null,

			extent == null ? null : new WhereClause.CompareColumn(COLUMN_EXTENT, ">=", (byte)extent)
		), limit, orderBy ?? [new OrderByClause(COLUMN_EXTENT, OrderByClause.OrderBy.Descending)]);
	}
