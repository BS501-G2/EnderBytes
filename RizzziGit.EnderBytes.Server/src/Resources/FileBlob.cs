...
{
    public new sealed record Resource(long Id, long CreateTime, long UpdateTime, byte[] Buffer)
    ...
    public FileBlobManager(ResourceService service)
        : base(service, NAME, VERSION)
    {
        GetManager<FileDataManager>().RegisterDeleteHandler(handler);
        GetManager<FileDataManager>()
            .RegisterUpdateHandler((transaction, _, old) => handler(transaction, old));

        async Task handler(
            ResourceService.Transaction transaction,
            FileDataManager.Resource fileData
        )
        {
            ...
            if (referenceCount == 0)
            {
                await SqlNonQuery(
                    transaction,
                    $"delete from {NAME} where {COLUMN_ID} = {fileData.Id}"
                );
            }
        }
    }
    ...
    protected override Resource ToResource(
        ...
        long createTime,
        long updateTime
    ) => new(id, createTime, updateTime, reader.GetBytes(reader.GetOrdinal(COLUMN_BUFFER)));
    ...
    )
    {
        if (oldVersion < 1)
        {
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_BUFFER} mediumblob not null;"
            );
        }
    }

    public async Task<long> Store(
        ResourceService.Transaction transaction,
        KeyService.AesPair fileKey,
        byte[] buffer
    )
    {
        return await Insert(transaction, new((COLUMN_BUFFER, fileKey.Encrypt(buffer))));
    }
