...
public sealed class FileContentManager : ResourceManager
{
    ...
    public FileContentManager(ResourceService service)
    ...
    )
    {
        if (oldVersion < 1)
        {
            ...
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_IS_MAIN} integer(1) not null;"
            );
        }
    }

    public async Task<Resource> GetMainContent(
        ResourceService.Transaction transaction,
    ...
    )
    {
        ...
        {
            fileContent = await InsertAndGet(
                transaction,
                new((COLUMN_FILE_ID, file.Id), (COLUMN_IS_MAIN, true))
            );
    ...
    {
        Resource fileContent = await InsertAndGet(
            transaction,
            new((COLUMN_FILE_ID, file.Id), (COLUMN_IS_MAIN, false))
        );
