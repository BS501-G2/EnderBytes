...
using ResourceManager = ResourceManager<
    FileContentVersionManager,
    FileContentVersionManager.Resource
>;
...
public sealed class FileContentVersionManager : ResourceManager
{
    ...
    public FileContentVersionManager(ResourceService service)
        : base(service, NAME, VERSION)
    {
        GetManager<FileContentManager>()
    ...
    )
    {
        if (oldVersion < 1)
        {
            ...
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_BASE_VERSION_ID} integer null;"
            );
        }
    }
