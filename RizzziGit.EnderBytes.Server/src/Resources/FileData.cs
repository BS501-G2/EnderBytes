...
public sealed class FileDataManager : ResourceManager
{
    ...
    public FileDataManager(ResourceService service)
        : base(service, NAME, VERSION)
    {
        GetManager<FileContentVersionManager>()
            .RegisterDeleteHandler(
                async (transaction, fileContentVersion) =>
                {
                    await Delete(
                        transaction,
                        new WhereClause.Nested(
                            ...
                            new WhereClause.CompareColumn(
                                COLUMN_FILE_CONTENT_VERSION_ID,
                                "=",
                                fileContentVersion.Id
                            )
                        )
                    );
                }
    ...
    )
    {
        if (oldVersion < 1)
        {
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_FILE_ID} integer not null;"
            );
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_FILE_CONTENT_ID} integer not null;"
            );
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_FILE_CONTENT_VERSION_ID} integer not null;"
            );
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_FILE_BLOB_ID} integer not null;"
            );
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_INDEX} integer not null;"
            );
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_SIZE} integer not null;"
            );
        }
    }

    public async Task<long> GetExclusiveMaxIndex(
        ResourceService.Transaction transaction,
    ...
    {
        return (
                await SqlScalar<long>(
                    transaction,
                    $"select max({COLUMN_INDEX}) from {NAME} where {COLUMN_FILE_ID} = {{0}} and {COLUMN_FILE_CONTENT_ID} = {{1}} and {COLUMN_FILE_CONTENT_VERSION_ID} = {{2}};",
                    file.Id,
                    fileContent.Id,
                    fileContentVersion.Id
                )
    ...
    {
        long maxIndex = await GetExclusiveMaxIndex(
            transaction,
            file,
            fileContent,
            fileContentVersion
    ...
    )
    {
        ...
        {
            Resource? fileData = await SelectFirst(
                transaction,
                new WhereClause.Nested(
                    "and",
                    new WhereClause.CompareColumn(COLUMN_FILE_ID, "=", file.Id),
            ...
            CompositeBuffer buffer =
                fileData == null
                    ...
                    : await GetManager<FileBlobManager>()
                        .Retrieve(transaction, fileKey, fileData.BlobId);
    ...
    )
    {
...

        {
            ...
            for (long index = startIndex; index < endIndex; index++)
            {
                Resource? fileData = fileDataMap
                    .Where((mapEntry) => mapEntry.Index == index)
                    .Select((e) => e.FileData)
                    .FirstOrDefault();
                ...
                CompositeBuffer toWrite = bytes.SpliceStart(
                    long.Min(buffer.Length - bufferStart, bytes.Length)
                );
                buffer.Write(bufferStart, toWrite);

                long newBlobId = await GetManager<FileBlobManager>()
                    .Store(transaction, fileKey, buffer.ToByteArray());
