...
public sealed partial class FileResource
{
  public sealed partial class ResourceManager
  {
    private sealed record FileHandle : FileResource.FileHandle, IDisposable
    {
      ...
      {
        return transaction.ResourceService.GetManager<FileBufferMapResource.ResourceManager>().GetSize(transaction, Storage, File, Snapshot, cancellationToken);
      }
      ...
      public override CompositeBuffer Read(ResourceService.Transaction transaction, int size, CancellationToken cancellationToken = default)
      {
        lock (this)
        {
          ...
          CompositeBuffer buffer = transaction.ResourceService.GetManager<FileBufferMapResource.ResourceManager>().Read(transaction, Storage, File, Snapshot, Position, size, UserAuthenticationToken, cancellationToken);
      ...
      public override void Write(ResourceService.Transaction transaction, CompositeBuffer buffer, CancellationToken cancellationToken = default)
      {
        lock (this)
        {
          ...
          transaction.ResourceService.GetManager<FileBufferMapResource.ResourceManager>().Write(transaction, Storage, File, Snapshot, Position, buffer, UserAuthenticationToken, cancellationToken);
          Position += buffer.Length;
      ...
      public override void Truncate(ResourceService.Transaction transaction, long size, CancellationToken cancellationToken = default)
      {
        lock (this)
        {
          ...
          transaction.ResourceService.GetManager<FileBufferMapResource.ResourceManager>().Truncate(transaction, Storage, File, Snapshot, size, UserAuthenticationToken, cancellationToken);
          CurrentSize = InternalGetSize(transaction, cancellationToken);
        }
      }
    }
