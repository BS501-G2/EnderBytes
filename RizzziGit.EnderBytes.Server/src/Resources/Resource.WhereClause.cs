...
public abstract partial class ResourceManager<M, R>
{
    protected abstract record WhereClause
    {
        ...
        public sealed record CompareColumn(string Column, string Comparer, object? Value)
            : WhereClause
        {
            public override string Apply(List<object?> parameterList)
            {
        ...
        public sealed record Nested(string Connector, params WhereClause?[] Expressions)
            : WhereClause
        {
            public override string Apply(List<object?> parameterList)
            {
                WhereClause[] expressions = Expressions
                    .Where((e) => e != null)
                    .Select((e) => e!)
                    .ToArray();
