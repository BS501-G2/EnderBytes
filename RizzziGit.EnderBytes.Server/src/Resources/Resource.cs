	...
	where R : ResourceManager<M, R>.Resource
{
	...
	protected async IAsyncEnumerable<R> Select(ResourceService.Transaction transaction, WhereClause? where = null, LimitClause? limit = null, OrderByClause[]? order = null)
	{
		List<object?> parameterList = [];
	...
	protected async Task<long> Update(ResourceService.Transaction transaction, WhereClause where, SetClause set)
	{
		...
		foreach (Func<Task> callback in await SqlQuery(
			...
			(reader) =>
			{
				...
				async Task<R> getNewResource() => await SqlQuery(transaction, (reader) => Task.FromResult(ToResource(reader)), $"select * from {Name} where {COLUMN_ID} = {{0}};", oldResource.Id).FirstAsync();
