...
public sealed class StorageResource(StorageResource.ResourceManager manager, UserResource user, StorageResource.ResourceData data) : Resource<StorageResource.ResourceManager, StorageResource.ResourceData, StorageResource>(manager, data)
{
  ...
  private KeyService.AesPair? StorageKey;

  public byte[] EncryptFileKey(KeyService.AesPair key, UserAuthenticationResource.UserAuthenticationToken userAuthenticationToken)
  {
    lock (this)
    {
      ThrowIfInvalid();

      StorageKey ??= KeyService.AesPair.Deserialize(userAuthenticationToken.Decrypt(Key));

      return StorageKey.Encrypt(key.Serialize());
    }
  }

  public KeyService.AesPair DecryptFileKey(FileResource file, UserAuthenticationResource.UserAuthenticationToken userAuthenticationToken)
  {
    lock (this)
    {
      ThrowIfInvalid();
      ...
      lock (file)
      {
        ...
        lock (userAuthenticationToken)
        {
          ...
          if (OwnerUserId != userAuthenticationToken.UserId)
          {
