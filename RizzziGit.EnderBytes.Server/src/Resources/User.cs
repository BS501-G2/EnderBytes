...
public sealed record UserPair(
    UserManager.Resource User,
    UserAuthenticationToken AuthenticationToken
);

public sealed partial class UserManager(ResourceService service)
    : ResourceManager<UserManager, UserManager.Resource>(service, NAME, VERSION)
{
    public sealed class UsernameNotFoundException(string Username)
        : Exception($"Username not found: {Username}.");
    ...
    )
    {
        if (oldVersion < 1)
        {
            await SqlNonQuery(
                transaction,
                $"alter table {Name} add column {COLUMN_USERNAME} varchar(16) not null collate nocase;"
            );
            await SqlNonQuery(
                transaction,
                $"alter table {Name} add column {COLUMN_PUBLIC_KEY} blob null;"
            );
            await SqlNonQuery(
                transaction,
                $"alter table {Name} add column {COLUMN_LAST_NAME} varchar(32) not null;"
            );
            await SqlNonQuery(
                transaction,
                $"alter table {Name} add column {COLUMN_FIRST_NAME} varchar(32) not null;"
            );
            await SqlNonQuery(
                transaction,
                $"alter table {Name} add column {COLUMN_MIDDLE_NAME} varchar(32) null;"
            );

            await SqlNonQuery(
                transaction,
                $"create unique index {INDEX_USERNAME} on {Name}({COLUMN_USERNAME});"
            );
        }
    }

    public async Task<UserPair> Create(
        ResourceService.Transaction transaction,
        string username,
        string lastName,
        string firstName,
    ...
    )
    {
        ...
        Resource user = await InsertAndGet(
            transaction,
            new(
        ...
        return new(
            user,
            await Service
                .GetManager<UserAuthenticationManager>()
                .CreatePassword(transaction, user, password, privateKey, publicKey)
        );
    }
    ...
    public async Task<Resource?> GetByUsername(
        ResourceService.Transaction transaction,
        string username
    )
    {
        ...
        return await SelectFirst(
            transaction,
            new WhereClause.CompareColumn(COLUMN_USERNAME, "=", username)
        );
    }

    public async Task<long> CountUsers(ResourceService.Transaction transaction) =>
        (long)(await SqlScalar(transaction, $"select count(*) from {NAME};"))!;
}
