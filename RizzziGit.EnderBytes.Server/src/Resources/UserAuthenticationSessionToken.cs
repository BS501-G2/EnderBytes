...
public sealed class UserAuthenticationSessionTokenManager
    : ResourceManager<
        UserAuthenticationSessionTokenManager,
        UserAuthenticationSessionTokenManager.Resource
    >
{
    public new sealed record Resource(
        ...
        : ResourceManager<UserAuthenticationSessionTokenManager, Resource>.Resource(
            Id,
            CreateTime,
            UpdateTime
        )
    ...
    public UserAuthenticationSessionTokenManager(ResourceService service)
    ...
    )
    {
        if (oldVersion < 1)
        {
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_USER_AUTHENTICATION_ID} integer not null;"
            );
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_EXPIRY_TIME} integer not null;"
            );
        }
    }
    ...
    )
    {
        if (userAuthentication.Type != UserAuthenticationType.SessionToken)
        {
            throw new ArgumentException(
                "Invalid user authentication type.",
    ...
    public async Task<Resource> GetByUserAuthentication(
        ResourceService.Transaction transaction,
        UserAuthenticationManager.Resource userAuthentication
    )
    {
    ...
    public async Task<bool> ResetExpiryTime(
        ResourceService.Transaction transaction,
        Resource userAuthenticationSessionToken,
        long expireTimer
    )
    {
        return await Update(
            transaction,
            userAuthenticationSessionToken,
            new((COLUMN_EXPIRY_TIME, DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + expireTimer))
        );
    }
}
