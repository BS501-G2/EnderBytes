...
public sealed class UserConfigurationManager
    : ResourceManager<UserConfigurationManager, UserConfigurationManager.Resource>
{
    ...
    public UserConfigurationManager(ResourceService service)
    ...
    )
    {
        if (oldVersion < 1)
        {
            ...
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_ENABLE_FTP_ACCESS} integer not null;"
            );
    ...
    public async Task<Resource> Get(
        ResourceService.Transaction transaction,
