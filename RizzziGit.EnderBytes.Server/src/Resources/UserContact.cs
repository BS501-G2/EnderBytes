...
public sealed class UserContactManager : BaseManager
{
    public new sealed record Resource(
        ...
        bool IsVerified
    ) : BaseManager.Resource(Id, CreateTime, UpdateTime);
    ...
    public UserContactManager(ResourceService service)
    ...
    )
    {
        if (oldVersion < 1)
        {
            ...
            await SqlNonQuery(
                transaction,
                $"alter table {NAME} add column {COLUMN_IS_VERIFIED} integer(1) not null;"
            );
        }
    }
}
